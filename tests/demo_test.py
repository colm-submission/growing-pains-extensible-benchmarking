...
def run_fast() -> None:
    ...
    models = [f"m_{i}" for i in range(8)]
    dates = {f"m_{i}": f"2024-01-{i+1:02d}T00:00:00+00:00" for i in range(8)}
    train_r, test_r = split_reference_test_models(
        models, test_ratio=0.25, split_mode="random", seed=42
    )
    assert len(test_r) == 2 and len(train_r) == 6

    # efficbench noniid: newest-first → test is first 25%
    dates_newest = {f"m_{i}": f"2024-01-{8-i:02d}T00:00:00+00:00" for i in range(8)}
    train_t, test_t = split_reference_test_models(
        models, test_ratio=0.25, split_mode="noniid", model_dates=dates_newest,
    )
    assert test_t == {"m_0", "m_1"} and train_t == {f"m_{i}" for i in range(2, 8)}
