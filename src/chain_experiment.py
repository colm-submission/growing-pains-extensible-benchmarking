...
from src.model_splits import split_reference_test_models
...
def _load_task_data(task: ScenarioTask) -> tuple:
    ...
    target_ref_df = pd.read_pickle(task.target_ref_df_path) if task.distance >= 1 else pd.DataFrame()
    target_test_df = pd.read_pickle(task.target_test_df_path) if task.distance >= 1 else pd.DataFrame()
    ...
    return final_df, target_ref_df, target_test_df, anchor_items
...
def run_scenario_task(task: ScenarioTask, gpu_id: int | None = None) -> dict:
    ...
    final_df, target_ref_df, target_test_df, anchor_items = _load_task_data(task)
    ...
    anchor_counts = {
        ...
        for ds in sorted(target_test_df['dataset'].unique())
    }
    ...
    if not target_test_df.empty:
        base_seed = task.random_seed + task.distance * 100

        # Validation and baselines for each anchor method (held-out test models on target)
        for anchor_method, (anchors, weights) in anchors_by_method.items():
            ...
            thetas = precompute_thetas_from_all_anchors(
                test_df=target_test_df, item_params=irt_params,
                anchor_ids=anchors, A_matrix=A_matrix, B_matrix=B_matrix,
            )
            res = run_validation(target_test_df, irt_params, anchors, weights,
        ...
        disc, disc_pm = run_discriminative_baseline_validation(
            test_df=target_test_df, item_params=irt_params, n_anchors=task.n_anchors_per_dataset,
        ...
        rand_irt, rand_irt_pm = run_random_baseline_validation(
            test_df=target_test_df, item_params=irt_params, n_random_questions=task.n_anchors_per_dataset,
            target_name=task.target_name, train_df=final_df, A_matrix=A_matrix, B_matrix=B_matrix,
        ...
        rand_simple, rand_simple_pm = run_random_simple_baseline(
            test_df=target_test_df, target_name=task.target_name,
...
def _build_scenario_tasks(
    ...
    target_ref_df = target_df[target_df['model_name'].isin(train_models)].copy()
    target_test_df = target_df[target_df['model_name'].isin(test_models)].copy()
    target_ref_path = temp_dir / "target_ref.pkl"
    target_test_path = temp_dir / "target_test.pkl"
    target_ref_df.to_pickle(target_ref_path)
    ...
    for (distance, chain_str, chain_list, prev_irt_path, prev_A_path, prev_B_path,
        ...
        final_df = pd.concat([prev_df, target_ref_df], ignore_index=True)
        ...
        for method in [METHOD_FIXED, METHOD_CONCURRENT]:
            ...
            tasks.append(ScenarioTask(
                ...
                target_ref_df_path=str(target_ref_path),
...
def _train_test_split(config: ParallelChainConfig, datasets: dict, base_names: list,
    ...
    print(f"3. Train/test split ({config.split_mode})...")
    ...
    train_models, test_models = split_reference_test_models(
        ...
        seed=config.seed,
        assume_pickle_ordered=config.assume_pickle_ordered,
    )
    ref_frac = 1.0 - config.test_ratio
    print(
        f"   Reference: {len(train_models)} ({ref_frac:.0%}), "
        f"Test (held-out): {len(test_models)} ({config.test_ratio:.0%})"
    )
    ...
    config_dict = {
        ...
        'split_mode': config.split_mode, 'test_ratio': config.test_ratio,
        'assume_pickle_ordered': config.assume_pickle_ordered,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="random",
        choices=["random", "time_ordered", "noniid"],
        help="Model split: random 75/25, or efficbench temporal (time_ordered / noniid).",
    )
    parser.add_argument(
        "--assume-pickle-ordered",
        action="store_true",
        help="For time_ordered/noniid: models in input pickle are already newest-first (efficbench lb.pickle).",
    )
    parser.add_argument(
        "--test-ratio",
        type=float,
        default=0.25,
        help="Fraction of models held out for test (default 0.25 → 75%% reference).",
    )
    ...
    config = ParallelChainConfig(
        ...
        assume_pickle_ordered=args.assume_pickle_ordered,
    )
    ...
    if args.output_dir:
        ...
        split_tag = args.split_mode if args.split_mode == "random" else f"split_{args.split_mode}"
        config.output_dir = PROJECT_ROOT / "data" / (
            f"chain_{args.data_source_mode}_seed_{args.shuffle_seed}"
            f"_anchors_{args.n_anchors_per_dataset}_dims_{dims_str}_{split_tag}"
        )
