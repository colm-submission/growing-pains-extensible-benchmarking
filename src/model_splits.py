"""Reference vs test model splits (random or efficbench-style temporal / noniid).

Temporal split follows efficbench (``run_experiment.py`` + ``process_lb_data.ipynb``):
  1. Date per model from Open LLM Leaderboard evaluation revisions (preferred).
  2. If unavailable, fall back to Hugging Face model-repo metadata (``createdAt``).
  3. Sort models newest-first; test = first ``test_ratio`` fraction (newest).
"""
...
DEFAULT_DATES_CACHE = PROJECT_ROOT / "data" / "input" / "model_gsm8k_dates.json"
...
def split_reference_test_models(
    ...
    random:
        Uniform random 75/25 split (Growing Pains Section 4.1 default).
    ...
    if split_mode == "random":
        ...
        unique = sorted(set(model_list))
        rng = np.random.default_rng(seed)
        test_models = set(rng.choice(unique, size=n_test, replace=False))
        train_models = set(unique) - test_models
    ...
    raise ValueError(
        f"Unknown split_mode {split_mode!r}. Expected 'random', 'time_ordered', or 'noniid'."
    )
