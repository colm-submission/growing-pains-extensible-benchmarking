#!/usr/bin/env bash
# One chain_experiment.py run per Slurm job. Override resources when submitting, e.g.:
...
VENV_ACTIVATE="${RUN_SWEEP_VENV:-${PROJECT_DIR}/.venv/bin/activate}"
...
elif [ -d "/path/to/resource" ]; then
  export HF_HOME="/path/to/resource"
fi
